...
let
  ...
  skopeo-nix2container = pkgs.skopeo.overrideAttrs (old: {
    EXTRA_LDFLAGS = l.optionalString pkgs.stdenv.hostPlatform.isDarwin "-X github.com/nlewo/nix2container/nix.useNixCaseHack=true";
  ...
  pullImageFromManifest =
    ...
    , registryUrl ? "registry-1.docker.io"
      ...
      getManifest = writeSkopeoApplication "get-manifest" ''
        manifest=$(skopeo inspect "${imageUrl}:${imageTag}" --raw)
