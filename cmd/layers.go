...
var layersReproducibleCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		layers, err := nix.NewLayersWithOptions(storepaths, maxLayers, nix.LayerOptions{Parents: parents, Rewrites: rewrites, Exclude: ignore, Perms: perms, Tars: tars}, history)
		if err != nil {
			fmt.Fprintf(os.Stderr, "%s", err)
			os.Exit(1)
		}
...
var layersNonReproducibleCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		layers, err := nix.NewLayersNonReproducibleWithOptions(storepaths, maxLayers, tarDirectory, nix.LayerOptions{Parents: parents, Rewrites: rewrites, Exclude: ignore, Perms: perms, Tars: tars}, history)
		if err != nil {
			fmt.Fprintf(os.Stderr, "%s", err)
			os.Exit(1)
		}
